...
{
/**
 * Orchestrion's notation interaction: a NotationInteraction that ignores the
 * selection and drag operations MuseScore's input controller would otherwise
 * perform on mouse events, since Orchestrion treats clicks on the score as
 * playback gestures rather than editing.
 */
...
public:
  ...
  using mu::notation::NotationInteraction::select;
